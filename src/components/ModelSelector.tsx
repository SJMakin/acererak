...
const ModelSelector: React.FC = () => {
  const { selectedModel, modelOptions, setSelectedModel, isLoading } = useModel();
    ...
    <div className="model-selector">
      ...
      >
        <div className="model-current-name">
          {selectedModel.name}
          {selectedModel.supportsJson && <span className="model-feature model-feature-json">JSON</span>}
          {selectedModel.isFree && <span className="model-feature model-feature-free">FREE</span>}
        </div>
                  ...
                  >
                    <div className="model-item-name">
                      ...
                      <div className="model-badges">
                        {model.supportsJson && <span className="model-feature model-feature-json">JSON</span>}
                        {model.isFree && <span className="model-feature model-feature-free">FREE</span>}
                      </div>
                    </div>
                  </div>
                    ...
                    <div className="model-item-description">
                      ...
                      >
                        {expandedModelId === model.id 
                          ? model.description 
                          : truncateText(model.description, MAX_DESCRIPTION_LENGTH)}
                      </div>
                      {model.description.length > MAX_DESCRIPTION_LENGTH && (
                        <button 
                          ...
                          onClick={() => toggleDescription(model.id)}
                        >
                          {expandedModelId === model.id ? 'Show less' : 'Show more'}
                        </button>
                      )}
                    </div>
                  )}
