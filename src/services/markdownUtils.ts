...
export function findAndReplaceMarkdownText(
  ...
  for (let i = 0; i < parts.length - 1; i++) {
    ...
    const startIndex =
      normalizedDocument.indexOf(beforePart, lastIndex) + beforePart.length;
    const endIndex = normalizedDocument.indexOf(parts[i + 1], startIndex);

    // Find the corresponding indices in the original document
    const originalStartIndex = findOriginalIndex(
      document,
      normalizedDocument,
      startIndex
    );
    const originalEndIndex = findOriginalIndex(
      document,
      normalizedDocument,
      endIndex
    );
    ...
    // Replace the text
    result =
      result.substring(0, originalStartIndex) +
      newText +
...
function isWhitespaceNormalized(
  ...
  if (/\s/.test(originalText[originalIndex])) {
    ...
    while (
      nextOriginalIndex < originalText.length &&
      /\s/.test(originalText[nextOriginalIndex])
    ) {
      nextOriginalIndex++;
    }
 ...
 */
export function dumpSheetAndSearchText(
  document: string,
  searchText: string,
  documentName: string = 'unknown'
): void {
