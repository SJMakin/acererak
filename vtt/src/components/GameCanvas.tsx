...
import { Stage, Layer, Rect, Line, Circle, Image, Text, Group } from 'react-konva';
import type Konva from 'konva';
import { nanoid } from 'nanoid';
import { useGameStore } from '../stores/gameStore';
import type { CanvasElement, Point, TokenElement, ImageElement, ShapeElement, TextElement } from '../types';
...
export default function GameCanvas({ room }: GameCanvasProps) {
  ...
  const handleMouseDown = useCallback((e: Konva.KonvaEventObject<MouseEvent | TouchEvent>) => {
    ...
    if (!isDrawingTool) {
      ...
      // Handle ping tool
      if (selectedTool === 'ping' && e.target === e.target.getStage()) {
        const stage = stageRef.current;
        if (stage) {
          const pointer = stage.getPointerPosition();
          if (pointer) {
  ...
  }, [isDrawingTool, selectedTool, drawStartPoint, room, viewportOffset, viewportScale]);
  ...
  const handleMouseUp = useCallback(() => {
    ...
    if (currentLine.length >= 4) {
      ...
      let newElement: Omit<ShapeElement, 'id'>;
