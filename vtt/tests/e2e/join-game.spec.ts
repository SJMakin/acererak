 ...
 * 
 * These tests verify:
 * 1. DM can create a game and get room ID
 * 2. Player can join using room ID
 * 3. P2P connection is established
 * 4. Game state syncs between DM and player
 */
...
test.describe('Join Game - P2P Flow', () => {
  ...
  test.beforeEach(async ({ browserName }) => {
    // Create separate browser contexts for DM and Player
    ...
    dmPage.on('console', msg => {
      ...
      console.log(`[DM ${msg.type()}] ${prefix} ${text}`);
    });
    ...
    dmPage.on('pageerror', error => {
      console.log(`[DM PAGE ERROR] ❌ ${error.message}`);
    });
  ...
  test('DM creates game and player joins via room ID', async () => {
    // Step 1: DM creates a game
    await dmPage.goto(BASE_URL);
    await expect(dmPage.getByRole('heading', { name: /Lychgate VTT/i })).toBeVisible();
    
    await dmPage.getByRole('tab', { name: /Create Game/i }).click();
    
    const gameName = `Test Game ${Date.now()}`;
    await dmPage.getByLabel(/Game Name/i).fill(gameName);
    await dmPage.getByPlaceholder('Dungeon Master').fill('Test DM');
    
    // Click create and wait
    await dmPage.getByRole('button', { name: /Create Game/i }).click();
    
    // Wait for game creation - this involves Trystero room setup
    // If this times out, P2P signaling is failing
    await expect(dmPage.getByText(/Game Created!/i)).toBeVisible({ timeout: 30000 });
    
    // Get the room ID
    const roomCode = dmPage.getByTestId('room-code');
    ...
    // DM starts the game (goes to canvas)
    await dmPage.getByRole('button', { name: /Start Game/i }).click();
    await expect(dmPage.locator('canvas').first()).toBeVisible({ timeout: 10000 });
  ...
  test('Player joins via URL with room parameter', async () => {
    ...
    await dmPage.getByLabel(/Game Name/i).fill(gameName);
    await dmPage.getByPlaceholder('Dungeon Master').fill('URL Test DM');
  ...
  test('DM sees player count update when player joins', async () => {
    // Step 1: DM creates a game
    await dmPage.goto(BASE_URL);
    await dmPage.getByRole('tab', { name: /Create Game/i }).click();
    
    await dmPage.getByLabel(/Game Name/i).fill('Player Count Test');
    await dmPage.getByPlaceholder('Dungeon Master').fill('Count DM');
    
    await dmPage.getByRole('button', { name: /Create Game/i }).click();
    await expect(dmPage.getByText(/Game Created!/i)).toBeVisible({ timeout: 30000 });
    
    // Initially should show 0 connected
    await expect(dmPage.getByText(/0 connected/i)).toBeVisible();
    ...
    // DM should see player count increase
    // Note: This requires P2P connection to establish
    await expect(dmPage.getByText(/1 connected/i)).toBeVisible({ timeout: 90000 });
  });

  test('Player sees "Player Joined" notification on DM side', async () => {
    // Step 1: DM creates and starts game
    await dmPage.goto(BASE_URL);
    await dmPage.getByRole('tab', { name: /Create Game/i }).click();
    await dmPage.getByLabel(/Game Name/i).fill('Notification Test');
    await dmPage.getByPlaceholder('Dungeon Master').fill('Notify DM');
    await dmPage.getByRole('button', { name: /Create Game/i }).click();
    await expect(dmPage.getByText(/Game Created!/i)).toBeVisible({ timeout: 30000 });
    ...
    // DM should see notification (Mantine notification)
    await expect(dmPage.getByText(/Player Joined/i)).toBeVisible({ timeout: 90000 });
    await expect(dmPage.getByText(/Notify Player.*joined/i)).toBeVisible({ timeout: 5000 });
  });
});
...
test.describe('Join Game - Error Handling', () => {
  test('should handle joining non-existent room gracefully', async ({ page }) => {
    ...
    // The P2P system will try to connect but no DM will respond
    ...
    // (since there's no DM to sync with)
...
test.describe('Join Game - Deployed Site Tests', () => {
  ...
  test('should navigate to create game tab and show form', async ({ page }) => {
    ...
    await expect(page.getByPlaceholder('Dungeon Master')).toBeVisible();
